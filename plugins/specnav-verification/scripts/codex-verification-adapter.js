...
const FULL_GATE_BLOCKER = 'codex-verification:full-gate-required';
const APPROVAL_BLOCKERS = new Map([
  ['runtime-setup', 'codex-verification:runtime-approval-required'],
  ['runtime-repair', 'codex-verification:runtime-approval-required'],
  ['migrate-apply', 'codex-verification:mutation-approval-required'],
  ['migrate-rollback', 'codex-verification:mutation-approval-required']
]);
...
function blocked(action, id, artifact = 'codex-adapter', detail = null) {
  const result = {
    ok: false,
    status: 'blocked',
    host: HOST,
...
function blockedSource(action, execution, id) {
...
function createCodexVerificationAdapter(options = {}) {
  if (typeof options.execute !== 'function') {
    throw new Error('codex-verification:executor-required');
  }
  ...
  function invoke(request = {}) {
    ...
    if (!SUPPORTED_ACTIONS.has(action)) {
      return blocked(
        action || '<missing>',
        `codex-verification:unsupported-action:${action || '<missing>'}`
      );
    }
    if (!isFullGateRequest(request)) {
      return blocked(action, FULL_GATE_BLOCKER, 'verification-policy');
    }
    const approvalBlocker = APPROVAL_BLOCKERS.get(action);
    if (approvalBlocker && request.approved !== true) {
      return blocked(
        action,
        approvalBlocker,
    ...
    if (
      ...
      return blocked(
        action,
        'codex-verification:project-root-required',
        'project-root'
      );
    }
    ...
    try {
      ...
      return blocked(
        action,
        'codex-verification:source-command-failed',
        'source-command',
...
function createProcessExecutor(pluginRoot = path.resolve(__dirname, '..')) {
    ...
    try {
      ...
      return {
        ...
        result: {
          ok: false,
          blockers: [{
            id: `codex-verification:project-root-missing:${projectRoot}`,
    ...
    if (!stat.isDirectory()) {
      return {
        ...
        result: {
          ok: false,
          blockers: [{
            id: `codex-verification:project-root-not-directory:${projectRoot}`,
            artifact: 'project-root',
            detail: null
          }]
        }
    ...
    try {
      ...
      return {
        ...
        result: {
          ok: false,
          blockers: [{
            id: 'codex-verification:source-output-invalid',
