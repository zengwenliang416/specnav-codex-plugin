# Task Brief: 024-overview-report

## Goal

Stakeholder can review lifecycle readiness, six domains, blockers, freshness, integrity, and repair status in overview.html.
...
Stakeholder can review lifecycle readiness, six domains, blockers, freshness, integrity, and repair status in overview.html. The delivered slice must be directly demonstrable through the
listed verification commands and must preserve all earlier artifacts.

## In Scope

- Render the approved overview prototype structure for green, red, blocked, running, canceled, stale, flaky, and pass-after-fix states.
...
- Prototype structure guides behavior; production renderer reimplements it under the report model contract.
...
- Approved prototype copy and token names
- Report model

## Components To Extract

- Shared navigation, status, metric, blocker, and timeline components

## API / Data Flow Contracts

- Capability spec: `openspec/changes/verification-2-0/specs/verification-report-center/spec.md`.
- Acceptance assertions: `AC-08`, `AC-11`, `AC-12`, `AC-29`.
- Report model renders overview.html with source references and projection disclaimer.
...
- `node --test tests/verification-v2/reports/overview.test.js`
- `bash tests/run-verification-v2-report-overview.sh`

## Stop Conditions

- Stop if a dependency task is incomplete: `023-report-model`.
...
Task packet path: `openspec/changes/verification-2-0/development/tasks/024-overview-report`.
